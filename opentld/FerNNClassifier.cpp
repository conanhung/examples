...
void FerNNClassifier::trainF(const vector<std::pair<vector<int>,int> >& ferns,int resample){
  ...
  for (size_t i = 0; i < ferns.size(); i++){               //   for (int i = 0; i < step; i++) {
    //     for (int k = 0; k < 10; k++) {
...
void FerNNClassifier::trainNN(const vector<cv::Mat>& nn_examples){
  ...
  y[0]=1;
  vector<int> isin;
  for (size_t i=0;i<nn_examples.size();i++){                          //  For each example
...
void FerNNClassifier::showpEx(){
  ...
  imshow("pExamples",examples);
}

void FerNNClassifier::shownEx(){
  Mat examples((int)nEx.size()*nEx[0].rows,nEx[0].cols,CV_8U);
  double minval;
  Mat ex(nEx[0].rows,nEx[0].cols,nEx[0].type());
  for (size_t i=0;i<nEx.size();i++){
    minMaxLoc(nEx[i],&minval);
    nEx[i].copyTo(ex);
    ex = ex-minval;
    Mat tmp = examples.rowRange(Range(i*nEx[i].rows,(i+1)*nEx[i].rows));
    ex.convertTo(tmp,CV_8U);
  }
  imshow("nExamples",examples);
}
